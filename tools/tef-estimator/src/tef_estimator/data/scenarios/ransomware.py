...
First and most developed scenario. All ransomware-specific data is loaded
from JSON files: IRIS reference data, Beazley vector proportions, and
scenario-derived parameters. Nothing is hardcoded.
"""
...
from tef_estimator.data.loader import load_reference, load_scenario, pert_from_list


def _load_ransomware_data() -> dict:
    """Load all ransomware-specific data from JSON files."""
    iris = load_reference("iris")
    beazley = load_reference("beazley")
    scenario = load_scenario("ransomware")

    # Vector proportions — multi-source (Beazley, Verizon, Unit42, Mandiant, CrowdStrike, Sophos)
    vector_proportions = {
        k: PERTRange(*pert_from_list(v))
        for k, v in beazley["vector_proportions"].items()
        if not k.startswith("_")
    }

    # Base rate triangulation — multi-source consensus
    base_rate = {
        k: PERTRange(*pert_from_list(v))
    ...
    # Overall ransomware share — IRIS Table 1
    overall_share = iris["ransomware_overall_share"]["value"]

    # Sector shares — IRIS Ransomware Fig 13
    sector_shares: dict[Sector, float | None] = {}
    for sector_key, val in iris["ransomware_sector_shares"].items():
        if sector_key.startswith("_"):
            continue
        sector_shares[Sector(sector_key)] = val

    # Sector loss shares — IRIS Ransomware Fig 13
    sector_loss_shares: dict[Sector, float | None] = {}
    for sector_key, val in iris["ransomware_sector_loss_shares"].items():
        if sector_key.startswith("_"):
            continue
        sector_loss_shares[Sector(sector_key)] = val

    # Revenue shares — IRIS Ransomware Fig 15
    revenue_shares: dict[RevenueBand, float] = {}
    for band_key, val in iris["ransomware_revenue_shares"].items():
        if band_key.startswith("_"):
            continue
        revenue_shares[RevenueBand(band_key)] = val

    # Revenue prob >=2 — IRIS Ransomware Table 3
    revenue_prob_gte2: dict[RevenueBand, float | None] = {}
    for band_key, val in iris["ransomware_revenue_prob_gte2"].items():
        if band_key.startswith("_"):
            continue
        revenue_prob_gte2[RevenueBand(band_key)] = val

    # Credential tempo — derived from Recorded Future + IR reports
    ...
    # Exploitation scanning — GreyNoise-derived
    ...
    return {
        ...
        "sector_loss_shares": sector_loss_shares,
...
_DATA = _load_ransomware_data()


class RansomwareScenario:
    """Ransomware threat scenario definition."""
    ...
    def active_vectors(self) -> list[str]:
        return ["exploitation", "credential", "phishing", "supply_chain"]
    ...
    def sector_loss_shares(self) -> dict[Sector, float | None]:
        return _DATA["sector_loss_shares"]
